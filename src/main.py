...
def generate_page_recursive(dir_path_content, template_path, dest_dir_path, basepath):
    ...
    for path in paths:
        ...
        if os.path.isfile(new_path):
            generate_page(new_path,template_path,os.path.join(dest_dir_path,path),basepath)
            continue
        if os.path.isdir(new_path):
            os.mkdir(os.path.join(dest_dir_path, path))
            generate_page_recursive(new_path,template_path,os.path.join(dest_dir_path, path), basepath)
